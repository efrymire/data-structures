...
// ----------------------- PARSE -----------------------
// parse the data and fill three arrays: (1) the entire left column cell, 
// (2) the street address, and (3) the meeting specific details
...
// (2) wheelchair details even if undefined, (3) notes even if undefined
    ...
    function jsonMeetingNotation(input) {
        ...
        for (var j in input) {
            ...
            if (input[j].split(' // ')[1].slice(-2) == 'PM') {
                var hour = Number(input[j].split(' // ')[1].slice(-8,-6).trim()) + 12
                var min = input[j].split(' // ')[1].slice(-5,-2).trim()
                thisMeeting.start = hour + ':' + min
            } else {
                var hour = input[j].split(' // ')[1].slice(-8,-6).trim()
                var min = input[j].split(' // ')[1].slice(-5,-2).trim()
                thisMeeting.start = hour + ':' + min
            }
            ...
            if (input[j].split(' // ')[2].slice(-2) == 'PM') {
                var hour = Number(input[j].split(' // ')[2].slice(-8,-6).trim()) + 12
                var min = input[j].split(' // ')[2].slice(-5,-2).trim()
                thisMeeting.end = hour + ':' + min
            } else {
                var hour = input[j].split(' // ')[2].slice(-8,-6).trim()
                var min = input[j].split(' // ')[2].slice(-5,-2).trim()
                thisMeeting.end = hour + ':' + min
            }
