...
// api: AIzaSyDKkrhAAXDaYkyoamN8Mof_7DHNYCyLnDs
...
async.eachSeries(addressArray, function(value, callback) {
    ...
    // JSON string couldnt be translated into a text file, so by
    // including JSON.stringify it becomes a JSON string
    
    fs.writeFileSync('meetingsdata.txt', JSON.stringify(meetingsData));
    
    // var meetingsString = meetingsData;
    // var meetingsArray = meetingsString.split('},{');
    
    // var meetingsParsed = JSON.parse(meetingsString);
    
    // var meetingsArray = [meetingsParsed.address + meetingsParsed.latlong + meetingsParsed.formattedAddress];
   
    // console.log(meetingsData);
    
    // console.log(meetingsString);
